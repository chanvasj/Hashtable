...
HashTable::HashTable() {
    // Initialize all pairs in the table with empty key "" and value -1
...
void test_overwrite_collision() {
    ...
    ht.insert("test", 1);
    std::cout << "Inserted ('test', 1)" << std::endl;

    // Overwrite with a new value
    ht.insert("test", 2);
    std::cout << "Overwrote ('test', 2)" << std::endl;

    // Check the value
    if (ht.contains("test")) {
        int value = ht.get("test");
        if (value == 2) {
            std::cout << "Key 'test' was correctly overwritten with value 2." << std::endl;
        } else {
            std::cerr << "Error: Key 'test' has incorrect value " << value << std::endl;
        }
    } else {
        std::cerr << "Error: Key 'test' was not found in the hashtable." << std::endl;
    }
}
